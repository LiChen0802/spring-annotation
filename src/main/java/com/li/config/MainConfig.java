...
/**
 * FilterType  按照指定形式进行指定规则<p>
 * ANNOTATION, 根据注解<p>
 * ASSIGNABLE_TYPE, 根据给定的类型(.class)<p>
 * ASPECTJ, ASPECTJ表达式<p>
 * REGEX, 正则表达式<p>
 * CUSTOM, 自定义规则<p>
 */
@Configuration
...
public class MainConfig {
    ...
    /**
     * Scope  : 调整类的作用域<p>
     * prototype :设定对象为多实例 ioc容器启动时不会调用方法 创建对象放在容器中<p>
     * 每次获取对象时才会创建对象
     * singleton : (默认值) 单实例 ioc容器启动时就会将对象放入容器中<p>
     * 每次获取都是从容器中调取
     * request : 同一次请求 获取相同相同对象<p>
     * session : 同一会话 获取相同相同对象<p>
